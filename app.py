...
# -------------------------------
# Improved Prediction Function (realistic look)
# -------------------------------
def predict_future(last_value, days):
    ...
    for i in range(days):
        ...
        noise = np.random.normal(0, 0.01)  # randomness
...
if st.button("Predict Future Prices"):
    ...
    # Plot last 20 real points for context
    recent_data = data['Close'].tail(20).values
    ax.plot(range(-19,1), recent_data, label="Recent Prices")

    # Plot predicted future
    ax.plot(range(1, days+1), future_prices, marker='o', label="Predicted")
